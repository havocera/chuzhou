...
export default {
  ...
  created(){
this.loginForm.auth=this.options[1].value
  },
  methods: {
    getchange(e){
        console.log(e)
        if(e==2){
          this.loginForm.name="银行管理员"
        }else if(e==1){
    ...
    login() {
      ...
      this.$http.post('login',{"name":this.loginForm.name, "pwd":this.loginForm.pwd}).then(res => {
        ...
        } else if (res.data.data.groupname == '银行管理员') {
          console.log('2')
...
.login_content {
  width: 100%;
  height: 100%;
  // background: url('../assets/images/jzbg2.png') no-repeat;
